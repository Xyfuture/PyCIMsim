...
from sim.core.compo.connection.payloads import TransferInfo, BusPayload, MemoryRequest, InterCoreMessage
...
class TransferUnit(BaseCoreCompo):
    ...
    def process(self):
        ...
        if op == 'send':
            self.cur_send_coroutine = self.process_send(transfer_info)
            next(self.cur_recv_coroutine)
        elif op == 'recv':
            self.cur_recv_coroutine = self.process_receive(transfer_info)
            next(self.cur_recv_coroutine)
        else:
            # 内存操作有关指令
            self.cur_memory_coroutine = self.process_memory(transfer_info)
    ...
    def process_receive(self, transfer_info: TransferInfo):
        ...
        yield ('receive data from src core'
               'execution finish')

...
class DummyLocalBuffer(BaseCoreCompo):
    def __init__(self, sim):
        super(DummyLocalBuffer, self).__init__(sim)
    ...
    def setup_request(self, node_id):
        ...
        3 for transfer
        '''
        payload = self._ports[node_id].read(self.current_time)
        print("Memory receive request : Node:{} Payload:{} {}".format(node_id, payload.payload, self.current_time))
    ...
    def process_request(self):
        if not self._busy:
            if not self._request_queue.empty():
                ...
                print("Memory process request : Node:{} Payload:{} {}".format(node_id, payload.payload,
                                                                              self.current_time))
                ...
                f = lambda: self.finish_process(payload.payload, node_id)
